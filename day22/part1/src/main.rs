...
fn main() {
    ...
    println!("Part 1: {}", virus.infect_count);
}
    ...
    fn from_start_grid(grid: &Vec<Vec<bool>>) -> Self {
        ...
        for (y, row) in grid.iter().enumerate() {
            for (x, val) in row.iter().enumerate() {
                if *val {
                    infected.insert((x as isize - size / 2, y as isize - size / 2));
                }
            }
        }
